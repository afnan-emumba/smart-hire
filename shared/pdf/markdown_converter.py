...
class PdfMarkdownConverter:
    """Extracts text from a PDF and normalizes it into lightweight markdown.

    Subclasses provide a domain-specific `_SECTION_HEADINGS` map (case-insensitive
    heading text -> canonical `## Heading`) and an `_EMPTY_TEXT_ERROR` message.
    """

    _SECTION_HEADINGS: dict[str, str] = {}
    _EMPTY_TEXT_ERROR = "Unable to extract text from the uploaded PDF"
    _NO_CONTENT_ERROR = "Uploaded PDF does not contain readable text"
    ...
    def convert_pdf_to_markdown(cls, file_bytes: bytes) -> str:
        ...
        if not raw_text:
            raise ValueError(cls._EMPTY_TEXT_ERROR)

        normalized_lines = cls._normalize_lines(raw_text)
        if not normalized_lines:
            raise ValueError(cls._NO_CONTENT_ERROR)
